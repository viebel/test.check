(defproject org.clojure/test.check "0.9.1-SNAPSHOT"
  ...
  :codox {:namespaces [clojure.test.check
          ...
          :themes [:default [:klipse {:klipse/external-libs  "https://raw.githubusercontent.com/clojure/test.check/master/src/main/clojure"
                                      :klipse/require-statement "(ns my.test
                                                                  (:require [clojure.test.check :as tc :refer [quick-check]]
                                                                            [clojure.test.check.generators :as gen]
                                                                            [clojure.test.check.properties :as prop :include-macros true :refer [for-all]]))"}]]
          }
  :cljsbuild
  {:builds
   [{:id "node-dev"
